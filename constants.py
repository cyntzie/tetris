...
# gameplay
HORIZONTAL_MOVE_RATE = 15 # the number of pixels a piece moves in one frame when moving horizontally
FALL_RATE = 2 # the number of pixels a piece moves down every frame (can be a float, it is rounded)
PROGRESSIVE_SPEED_INCREASE = 0.5 # when difficulty is increased this is added to FALL_RATE
DIFFICULTY_INCREASE_RATE = 4 # the number of lines in between when difficulty is increased
...
LEFT_KEYS = [K_LEFT, K_KP4, K_a]
RIGHT_KEYS = [K_RIGHT, K_KP6, K_d]
UP_KEYS = [K_UP, K_KP8, K_w]
DOWN_KEYS = [K_DOWN, K_KP5, K_KP2, K_s]

# misc.
TILE_SIZE = (25, 25) # the size of the tiles and bricks, in pixels
WINDOW_CAPTION = 'Tetukka'
SCREEN_SIZE = (800, 600)
FPS = 50
...
ENTRY_MAX_NAME_LEN = 6
ENTRY_FONT_SIZE = 18
ENTRY_CURSOR_WIDTH = 3
ENTRY_CURSOR_COLOR = (255, 255, 255)
ENTRY_TEMPLATE = 'Your Name: %s '
ENTRY_CURSOR_BLINK_RATE = 1 

# default high score lists
DEFAULT_LINES_LIST = [(x, 'Joni') for x in range(NUM_HIGH_SCORES)] 
DEFAULT_SCORE_LIST = [(x * POINTS_PER_LINE, 'Joni') for x in range(NUM_HIGH_SCORES)]
