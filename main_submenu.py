...
class main_submenu(animated_background):
	'''
        Contains the things that are in the main menu.
	'''
	def __init__(self, main, screenrect):
		...
		self.highbutton = TextWidget(text = 'Best Scores', color = BUTTON_COLOR, font_filename = resources.fontfilename)
	...
	def quit_click(self, event): 
		print >> debug.log, 'stopped playing'
		self.main.destroy()
